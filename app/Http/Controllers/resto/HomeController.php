...
class HomeController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $juals = Jual::where('waktu_pesan','>=',date('Y-m-d'))->whereIn('status_jual', $this->_arr_status_jual_map[$status_jual])->paginate();foreach($juals as $cur){
        ...
        $konsumen_tersering = AlamatKirim::whereRaw("is_default", [])->max('nama_penerima');
        return view('resto.home.index', compact('juals','status_jual','arr_status_jual','jual', 'rating_50', 'rating_semua','order_minggu_terakhir',
        'order_bulan_ini', 'konsumen_tersering'));
