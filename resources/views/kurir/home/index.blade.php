...
<div class="container">
 ...
 <option value="{{ $cur }}" @if($cur==$status_jual) selected @endif>{{ $cur }}</option>
...
{{ $cur->alamat_kirim->alamat }}</p>
 ...
 <div class="col-auto">
    <label for="konsumen_rate" class="form-label">Beri rating konsumen</label>
        </div>
            <div class="col-auto">
 ...
 <option value="{{ $rate }}" {{ $rate==5 ? 'selected' : '' }}>{{ $rate }}</option>
